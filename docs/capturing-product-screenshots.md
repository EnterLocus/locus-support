...
1. `place-picker` — choose a Room and View;
2. `virtual-space` — an authored Room with a browser window;
...
xcrun simctl launch --terminate-running-process "$SIMULATOR_ID" \
  com.enterlocus.locus.development --workspace-page places
sleep 5
xcrun simctl io "$SIMULATOR_ID" screenshot place-picker.png
...
xcrun simctl launch --terminate-running-process "$SIMULATOR_ID" \
  ...
  --safe-head-debug inside --quick-panel
sleep 12
...
screenshot. The deterministic fixtures make framing repeatable, but simulator
captures never replace physical Apple Vision Pro acceptance for tracking,
passthrough, occlusion, presence, performance, or comfort.
